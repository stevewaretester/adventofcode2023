...
for line in lines:#split by lines
    ...
    for round in rounds:#going round-by-round...
        ...
        sumRed = 0#we need to know all the reds.
        sumGreen = 0#all the greens
        sumBlue = 0#all the blues
        ...
        if (sumRed>maxRed) or (sumGreen>maxGreen) or (sumBlue>maxBlue): #if any colour is greater than its max-allowed number...
            impossGame = True #it was a bad game
            print(round+" was a bad round")
            break#if one round fails the entire game is impossible.
    
    if not impossGame: # if it was a possible game
        totalGood+=int(gameid)#take its ID and add it to the stack
        print("Good Game"+gameid+", totalgood: "+str(totalGood))
    else:
        print("Bad Game: "+gameid)

print("total sum of possible games: "+str(totalGood))